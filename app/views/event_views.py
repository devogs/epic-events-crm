...
from app.controllers.employee_controller import list_employees # Pour l'assignation de support
...
def display_event_table(events: List[Event], title: str):
    ...
    for event in events:
        client_name = event.contract.client.full_name if event.contract and event.contract.client else "N/A"
        support_info = f"{event.support_contact.full_name} ({event.support_contact_id})" if event.support_contact else "[bold red]UNASSIGNED[/bold red]"
        ...
        table.add_row(
            ...
            client_name,
            support_info,
            event.event_start.strftime('%Y-%m-%d %H:%M') if event.event_start else "N/A",
            event.location,
            str(event.attendees),
...
def list_events_cli(session: Session, current_employee: Employee):
    ...
    if current_employee.department == 'Support':
        ...
        if choice == '1':
            try:
                filter_by_support_id = int(Prompt.ask("Enter Support Employee ID").strip())
                title_suffix = f" (Filtered by Support ID {filter_by_support_id})"
            except ValueError:
                console.print("[bold red]ERROR:[/bold red] Invalid Support ID. Showing all events.")
                title_suffix = " (All Events)"
        else:
            title_suffix = " (All Events)"
        ...
        title_suffix = " (All Events)" # Should only be for Gestion if no filter is applied

    try:
        events = list_events(session, current_employee, filter_by_support_id=filter_by_support_id, support_filter_scope=filter_scope)
...
def update_event_cli(session: Session, current_employee: Employee):
    ...
    attendees_str = Prompt.ask("Enter New Number of Attendees (Leave empty to skip)").strip()
    if attendees_str:
        try:
            updates['attendees'] = int(attendees_str)
        except ValueError:
            console.print("[bold red]ERROR:[/bold red] Attendees must be a number. Skipping.")
    ...
    # Date fields
    start_date_str = Prompt.ask("Enter New Start Date (YYYY-MM-DD HH:MM) (Leave empty to skip)").strip()
    if start_date_str:
        try:
            updates['event_start'] = datetime.strptime(start_date_str, '%Y-%m-%d %H:%M')
        except ValueError:
            console.print("[bold red]ERROR:[/bold red] Invalid start date format (use YYYY-MM-DD HH:MM). Skipping.")

    end_date_str = Prompt.ask("Enter New End Date (YYYY-MM-DD HH:MM) (Leave empty to skip)").strip()
    if end_date_str:
        try:
            updates['event_end'] = datetime.strptime(end_date_str, '%Y-%m-%d %H:%M')
        except ValueError:
            console.print("[bold red]ERROR:[/bold red] Invalid end date format (use YYYY-MM-DD HH:MM). Skipping.")
    ...
    if current_employee.department == 'Gestion':
        new_contract_id_input = Prompt.ask("Enter New Contract ID (Gestion only, Leave empty to skip)").strip()
        if new_contract_id_input:
            try:
                updates['contract_id'] = int(new_contract_id_input)
            except ValueError:
                console.print("[bold red]ERROR:[/bold red] Contract ID must be a number. Entry cancelled.")
    
    # Support Contact ID (Support/Gestion only)
    if current_employee.department == 'Gestion' or current_employee.department == 'Support':
