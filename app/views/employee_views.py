...
def create_employee_cli(session, current_user: Employee) -> None:
    ...
    while True:
        ...
        options_list = [f"  [cyan]{k}[/cyan]: {v}" for k, v in ec.DEPARTMENT_OPTIONS.items()]
        console.print("\n".join(options_list))
        
        # Utilisation des clés numériques '1', '2', '3'
        dept_choice = Prompt.ask("Enter Department Key [1/2/3]", choices=ec.DEPARTMENT_OPTIONS.keys()).strip()
        
        if dept_choice in ec.DEPARTMENT_OPTIONS:
            department_name = ec.DEPARTMENT_OPTIONS[dept_choice]
            break
        else:
            console.print("[bold red]Invalid department key. Please try again.[/bold red]")
...
def list_employees_cli(session, current_user: Employee) -> None:
    ...
    if not auth.check_permission(current_user, 'view_employees'):
        console.print("[bold red]Permission denied.[/bold red] Only the 'Gestion' department can view employees.")
        return
...
def update_employee_cli(session, current_user: Employee) -> None:
    ...
    updates['full_name'] = Prompt.ask(f"Full Name (current: {employee.full_name})").strip() or None
    ...
    new_password = Prompt.ask("New Password (leave blank to keep current)", password=True).strip() or None
    if new_password:
    ...
    while True:
        ...
        # Utilisation des clés numériques '1', '2', '3'
        dept_choice = Prompt.ask(f"Enter New Department Key [1/2/3] (current: {employee.department})").strip()
    ...
    if not updates:
        console.print("[bold yellow]No changes detected. Aborting update.[/bold yellow]")
        return
