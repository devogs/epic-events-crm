...
from app.controllers.utils import is_valid_email, is_valid_phone # Nouvelle utilitaire pour le téléphone/email
...
def update_client(session: Session, current_user: Employee, client_id: int, **kwargs) -> Client | None:
    ...
    try:
        ...
        if 'sales_contact_id' in kwargs and kwargs['sales_contact_id']:
            ...
            # Vérification que le nouveau contact est un commercial
            new_sales_contact = session.query(Employee).filter(Employee.id==new_sales_id, Employee.role.has(Role.name=='Commercial')).one_or_none()
        ...
        session.rollback()
        # Capture toutes les erreurs (ValueError, PermissionError, IntegrityError, etc.)
        console.print(f"[bold red]ERREUR lors de la modification du client:[/bold red] {e}")
        return None